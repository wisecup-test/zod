# Standardize Schema Definition Patterns for Service API Boundaries: Schema Definitions Include
...
- **R-SCHEMA-001** MUST: Schema definitions MUST include comprehensive test coverage validating schema behavior (defaults, descriptions, transformations).

### Verify

```bash
# Find schema definitions using Zod or similar validation libraries
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Schema validation at service boundaries is mandatory for all in-scope API interfaces. Violations must be caught during code review and CI pipeline execution.
</enforcement>