...
1. SHOULD: Route handlers that serialize documentation for downstream consumption SHOULD cache derived page order mappings and metadata in-memory during request processing to prevent redundant source tree traversals.

## Policy Block

- SHOULD Route handlers that serialize documentation for downstream consumption SHOULD cache derived page order mappings and metadata in-memory during request processing to prevent redundant source tree traversals.
